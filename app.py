...
# Optional lightweight LLM imports
try:
...
def rule_based_generate(user_prompt):
    """Simple fallback generator if LLMs are not available"""
    return [
...
def gpt4all_generate(prompt):
    ...
    try:
        model = GPT4All("gpt4all-lora-quantized.bin")  # small CPU model
...
def llama_cpp_generate(prompt):
    if Llama is None:
        return rule_based_generate(prompt)
    try:
        model = Llama(model_path="llama-7b-q4.bin")  # small local model
...
st.title("Agentic AI API Engineer — Fortune 500 Style")
st.caption("Generate, try, and download simple APIs from requirements using lightweight open-source LLMs or fallback rules.")
...
if st.button("Generate API from scratch"):
    if choice == "Custom requirement":
        ...
        with st.spinner("Generating API endpoints using lightweight LLMs..."):
...
st.info("Cradle-to-cradle API engineering: requirement → OpenAPI → backend → client demo → simulated try → API Quality Score → download ZIP")
